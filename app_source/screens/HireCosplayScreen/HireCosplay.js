...
const HireCosplay = () => {
    ...
    const fetchAccountId = async () => {
      try {
        ...
        if (accessToken) {
          ...
          const accountId =
            decodedToken.Id || decodedToken.sub || decodedToken.userId;
          if (accountId) {
            setFormData((prevData) => ({ ...prevData, accountId }));
          } else {
            console.log("Không tìm thấy accountId trong token");
          }
        } else {
  ...
  const filterDateInput = (text) => text.replace(/[^0-9/]/g, "");
  const filterTimeInput = (text) => text.replace(/[^0-9:]/g, "");
      ...
      <ScrollView style={styles.content}>
        ...
        >
          <LinearGradient
            ...
            style={styles.sampleGradientButton}
          >
            <Text style={styles.sampleButtonText}>Fill Sample Data</Text>
          </LinearGradient>
        </TouchableOpacity>
